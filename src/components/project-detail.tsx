...
type ProjectActionEdit = Pick<ProjectAction, "title" | "targetDate" | "details"> & {
    ...
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-slate-50">
      ...
      <main className="mx-auto max-w-3xl px-4 pb-32 pt-7 sm:px-6">
        ...
        <section className="mt-8">
          ...
          {addingAction && !readOnly ? <AddActionForm onCancel={() => setAddingAction(false)} onAdd={(title, date, details) => { addProjectAction(project.id, title, date, details); setAddingAction(false); }} /> : null}
          {editingAction && !readOnly ? <EditActionForm action={editingAction} onCancel={() => setEditingActionId(null)} onSave={(title, date, details, note) => { const updates: ProjectActionEdit = { title, targetDate: date, details, rescheduleNote: note }; updateProjectAction(project.id, editingAction.id, updates); setEditingActionId(null); }} /> : null}
          {completingAction && !readOnly ? <CompleteActionForm action={completingAction} hasOtherOpenActions={openActions.length > 1} onCancel={() => setCompletingActionId(null)} onComplete={(note, nextTitle, nextDate, nextDetails) => { const resolution = nextTitle.trim() ? "next-action" : openActions.length > 1 ? "keep-active" : "waiting"; completeProjectAction(project.id, completingAction.id, note, resolution, nextTitle, nextDate, nextDetails); setCompletingActionId(null); }} /> : null}
        </section>
...
function AddActionForm({ onAdd, onCancel }: { onAdd: (title: string, date: string, details: string) => void; onCancel: () => void }) {
  const [title, setTitle] = useState(""); const [date, setDate] = useState(""); const [details, setDetails] = useState("");
  return <form className="mt-4 rounded-2xl border border-slate-200 bg-white p-4" onSubmit={(event) => { event.preventDefault(); onAdd(title, date, details); }}><p className="text-sm font-semibold text-slate-900">Add an open action</p><label className="mt-3 block text-sm font-medium text-slate-700">Action<input className="input mt-2" value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label className="mt-3 block text-sm font-medium text-slate-700">Details<textarea className="input mt-2 min-h-24 resize-y" value={details} onChange={(event) => setDetails(event.target.value)} placeholder={"Optional context, setup steps, bullets, or notes\n1. Run one provided example\n2. Record command, inputs, output, runtime, and setup problems"} /></label><label className="mt-3 block text-sm font-medium text-slate-700">Check-in date<input type="date" className="input mt-2" value={date} onChange={(event) => setDate(event.target.value)} /><span className="mt-2 block text-xs font-normal text-slate-500">Optional. Adding an action automatically makes a Waiting project Active.</span></label><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-10 rounded-xl px-3 text-sm font-semibold text-slate-600">Cancel</button><button type="submit" className="min-h-10 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white">Add action</button></div></form>;
}

function EditActionForm({ action, onSave, onCancel }: { action: ProjectAction; onSave: (title: string, date: string, details: string, note: string) => void; onCancel: () => void }) {
  const [title, setTitle] = useState(action.title); const [date, setDate] = useState(action.targetDate); const [details, setDetails] = useState(action.details ?? ""); const [note, setNote] = useState("");
  return <form className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4" onSubmit={(event) => { event.preventDefault(); onSave(title, date, details, note); }}><p className="text-sm font-semibold text-sky-950">Edit open action</p><label className="mt-3 block text-sm font-medium text-sky-950">Action<input className="input mt-2 bg-white" value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label className="mt-3 block text-sm font-medium text-sky-950">Details<textarea className="input mt-2 min-h-24 resize-y bg-white" value={details} onChange={(event) => setDetails(event.target.value)} placeholder="Optional context, setup steps, bullets, or notes" /></label><label className="mt-3 block text-sm font-medium text-sky-950">Check-in date<input type="date" className="input mt-2 bg-white" value={date} onChange={(event) => setDate(event.target.value)} /><span className="mt-2 block text-xs font-normal text-sky-800">Optional. Clearing it keeps the action open without a date.</span></label><label className="mt-3 block text-sm font-medium text-sky-950">Reschedule note<textarea className="input mt-2 min-h-20 resize-y bg-white" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional reason or context for the date change" /></label><p className="mt-3 text-xs leading-5 text-sky-800">Changing or removing the date keeps this action open and records the previous date and optional note.</p><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-10 rounded-xl px-3 text-sm font-semibold text-sky-900">Cancel</button><button type="submit" className="min-h-10 rounded-xl bg-sky-950 px-4 text-sm font-semibold text-white">Save action</button></div></form>;
}

function CompleteActionForm({ action, hasOtherOpenActions, onCancel, onComplete }: { action: ProjectAction; hasOtherOpenActions: boolean; onCancel: () => void; onComplete: (note: string, nextTitle: string, nextDate: string, nextDetails: string) => void }) {
  const [note, setNote] = useState("");
  const [nextTitle, setNextTitle] = useState(""); const [nextDate, setNextDate] = useState(""); const [nextDetails, setNextDetails] = useState("");
  return <form className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4" onSubmit={(event) => { event.preventDefault(); onComplete(note, nextTitle, nextDate, nextDetails); }}><p className="text-sm font-semibold text-emerald-950">Complete: {action.title}</p><label className="mt-3 block text-sm font-medium text-emerald-950">What happened?<textarea className="input mt-2 min-h-24 resize-y bg-white" value={note} onChange={(event) => setNote(event.target.value)} required /></label><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_11rem]"><label className="block text-sm font-medium text-emerald-950">Add a next action<input className="input mt-2 bg-white" value={nextTitle} onChange={(event) => setNextTitle(event.target.value)} placeholder="Optional" /></label><label className="block text-sm font-medium text-emerald-950">Check-in date<input type="date" className="input mt-2 bg-white" value={nextDate} onChange={(event) => setNextDate(event.target.value)} disabled={!nextTitle.trim()} /><span className="mt-2 block text-xs font-normal text-emerald-800">Optional.</span></label></div><label className="mt-3 block text-sm font-medium text-emerald-950">Next action details<textarea className="input mt-2 min-h-24 resize-y bg-white" value={nextDetails} onChange={(event) => setNextDetails(event.target.value)} placeholder="Optional context, setup steps, bullets, or notes" disabled={!nextTitle.trim()} /></label><p className="mt-3 text-xs leading-5 text-emerald-800">{hasOtherOpenActions ? "Leave the next action blank to continue with the other open actions." : "Leave the next action blank to move the project to Waiting automatically."}</p><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-10 rounded-xl px-3 text-sm font-semibold text-emerald-900">Cancel</button><button type="submit" className="min-h-10 rounded-xl bg-emerald-900 px-4 text-sm font-semibold text-white">Save completion</button></div></form>;
}
