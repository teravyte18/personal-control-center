...
const PHOTO_VARIANT_VERSION = "display-v1";
...
async function createDisplayPhoto(bytes: Uint8Array, originalMimeType: string) {
  try {
    ...
    console.warn("Could not optimize a recipe photo; serving the original upload.", error);
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const photo = await loadRecipePhoto(user.id, photoId);
    ...
    console.error("Could not load recipe photo.", error);
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    const user = await resolveRequestUser(request);
    const { photoId } = await context.params;
    await deleteRecipePhoto(user.id, photoId);
    return new Response(null, { status: 204 });
  } catch (error) {
    if (error instanceof AuthenticationRequiredError) return Response.json({ error: error.message }, { status: 401 });
    if (error instanceof RecipePhotoInputError) return Response.json({ error: error.message }, { status: error.status });
    console.error("Could not delete recipe photo.", error);
