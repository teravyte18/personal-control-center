    ...
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-slate-50">
      ...
      <main className="mx-auto max-w-3xl px-4 pb-32 pt-7 sm:px-6">
        ...
        <section className="mt-8">
          ...
          {editingAction && !readOnly ? <EditActionForm action={editingAction} onCancel={() => setEditingActionId(null)} onSave={(title, date) => { updateProjectAction(project.id, editingAction.id, { title, targetDate: date }); setEditingActionId(null); }} /> : null}
    ...
    <article className={`rounded-2xl border p-4 ${overdue ? "border-rose-300 bg-rose-50" : primary ? "border-slate-300 bg-white" : "border-slate-200 bg-white"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className={`mt-1 text-xs ${overdue ? "font-semibold text-rose-700" : "text-slate-500"}`}>{overdue ? "Check-in passed" : "Check in"} {formatDateOnly(action.targetDate)}</p>
        </div>
        {!readOnly ? <div className="flex shrink-0 gap-1"><button type="button" onClick={onEdit} className="min-h-10 rounded-xl px-3 text-xs font-semibold text-slate-500">Reschedule</button><button type="button" onClick={onComplete} className="min-h-10 rounded-xl bg-slate-950 px-3 text-xs font-semibold text-white">Complete</button></div> : null}
      </div>
...
function RescheduleHistory({ action, compact = false }: { action: ProjectAction; compact?: boolean }) {
  ...
  return <div className={compact ? "mt-3 border-t border-slate-100 pt-3" : "mt-2"}>{visible.map((entry, index) => <p key={`${entry.changedAt}-${index}`} className="text-xs leading-5 text-slate-500">{entry.previousTargetDate ? `Moved from ${formatDateOnly(entry.previousTargetDate)}` : "Date set"} to {formatDateOnly(entry.targetDate)} on {formatTimestamp(entry.changedAt)}</p>)}</div>;
}
