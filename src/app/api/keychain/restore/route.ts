...
import { restoreStoredKeychain } from "@/server/keychain-hardening-store";
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== "object") return json({ error: "Encrypted Keychain restore is invalid." }, 400);
  ...
  if (!Number.isInteger(expectedVaultRevision) || Number(expectedVaultRevision) < 1 || !vault || !records) {
    return json({ error: "Encrypted Keychain restore is invalid." }, 400);
  }

  try {
    const user = await resolveRequestUser(request);
    const restored = await restoreStoredKeychain(user.id, Number(expectedVaultRevision), vault, records);
    return json({ userId: user.id, ...restored });
  } catch (error) {
    ...
    console.error("Could not restore encrypted Keychain.");
    return json({ error: "Encrypted Keychain restore could not be saved." }, 503);
  }
}
