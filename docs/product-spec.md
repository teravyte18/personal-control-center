...
Important responsibilities, projects, interests, reference information, and future plans can live across memory and disconnected tools. This creates repeated mental review, makes it harder to focus, and allows useful ideas to compete with urgent work.
...
- Keep projects, one-off tasks, thoughts, editable notes, books, recurring routines, and future time-specific events conceptually distinct.
...
Capture is permanent in the centre. Four other slots are chosen from available pinnable spaces; the defaults are Inbox, Projects, Tasks, and Review. Thoughts, Notes, and Library are also pinnable. Configuration is stored per browser/device. A tappable upward handle beneath Capture opens All Spaces; no hidden gesture is required.
...
The Library and interface/theme slices are complete. No next major product slice is selected yet. Selection should follow real usage rather than treating the remaining modules as an automatic queue.
